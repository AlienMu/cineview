...
<!-- 手写目录，收二级与三级标题。改动标题后请一并更新：CONTRIBUTING.md 要求
     贡献者先读本文，而 2700 余行零导航本身就是阅读门槛。 -->

- [概述](#概述)
- [当前架构裁决](#当前架构裁决)
- [Drag Mode](#drag-mode)
  - [公共配置与继承](#公共配置与继承)
  - [双轨、纯元素时长与唯一映射](#双轨纯元素时长与唯一映射)
  - [Prepared snapshot 与 transaction 所有权](#prepared-snapshot-与-transaction-所有权)
  - [Candidate、re-grab suspension 与回调](#candidatere-grab-suspension-与回调)
  - [`driver='clock'` 的到场后驱动](#driverclock-的到场后驱动)
  - [预设中控与错误](#预设中控与错误)
  - [授权中的变体：authored-but-unparsed 必须停在 initial 帧](#授权中的变体authored-but-unparsed-必须停在-initial-帧)
  - [手动控制 + 兜底触发（`enterRef` / `exitRef`）](#手动控制--兜底触发enterref--exitref)
- [Scroll Mode](#scroll-mode)
  - [设计目标](#设计目标)
  - [公共语义](#公共语义)
  - [根节点模式设计](#根节点模式设计)
  - [Scene 规则](#scene-规则)
  - [Scene Scroll 规则](#scene-scroll-规则)
  - [Animate 规则](#animate-规则)
  - [时间轴与真实滚动距离规则](#时间轴与真实滚动距离规则)
  - [长动画规则](#长动画规则)
  - [Scene-Scoped Fixed Layer 规则](#scene-scoped-fixed-layer-规则)
  - [Scene 高度测量规则](#scene-高度测量规则)
  - [Root Scroll 运行规则](#root-scroll-运行规则)
  - [首屏与预加载规则](#首屏与预加载规则)
  - [Scrollbar 规则](#scrollbar-规则)
  - [产品体验裁决](#产品体验裁决)
  - [受影响的实现点](#受影响的实现点)
- [新模式参数原则](#新模式参数原则)
- [开发原则](#开发原则)
  - [1. 不靠猜测修问题](#1-不靠猜测修问题)
  - [2. 关键状态必须有唯一所有者（双轨模型，2026-06-25 起）](#2-关键状态必须有唯一所有者双轨模型2026-06-25-起)
  - [3. 关键日志必须覆盖 ownership handoff](#3-关键日志必须覆盖-ownership-handoff)
  - [4. 先恢复行为基线，再做架构迁移](#4-先恢复行为基线再做架构迁移)
- [架构](#架构)
  - [系统架构图](#系统架构图)
  - [组件层级关系](#组件层级关系)
  - [数据流图](#数据流图)
- [组件和接口](#组件和接口)
  - [组件 1: CineView（顶级容器组件）](#组件-1-cineview顶级容器组件)
  - [组件 2: Scene（场景组件）](#组件-2-scene场景组件)
  - [组件 2.5: Scene.scroll（局部滚动接管声明）](#组件-25-scenescroll局部滚动接管声明)
  - [模式说明](#模式说明)
  - [组件 3: Animate（动画组件）](#组件-3-animate动画组件)
  - [组件 3.5: AnimateVideo（原子时间轴与媒体所有权，2026-07-29）](#组件-35-animatevideo原子时间轴与媒体所有权2026-07-29)
  - [组件 4: Position（定位组件）](#组件-4-position定位组件)
- [数据模型](#数据模型)
  - [模型 1: CineViewContext](#模型-1-cineviewcontext)
  - [模型 2: SceneState](#模型-2-scenestate)
  - [模型 3: AnimationRegistry](#模型-3-animationregistry)
  - [模型 4: PreloadState](#模型-4-preloadstate)
- [错误处理](#错误处理)
  - [错误场景 1: 组件层级错误](#错误场景-1-组件层级错误)
  - [错误场景 2: 循环依赖检测](#错误场景-2-循环依赖检测)
  - [错误场景 3: 图片加载失败](#错误场景-3-图片加载失败)
  - [错误场景 4: 无效的场景索引](#错误场景-4-无效的场景索引)
- [测试策略](#测试策略)
  - [单元测试方法](#单元测试方法)
  - [属性测试方法](#属性测试方法)
  - [集成测试方法](#集成测试方法)
- [性能考虑](#性能考虑)
  ...
  - [代码分割和懒加载](#代码分割和懒加载)
  - [内存管理](#内存管理)
  - [Bundle 大小优化](#bundle-大小优化)
  - [首次内容绘制（FCP）优化](#首次内容绘制fcp优化)
  - [累积布局偏移（CLS）优化](#累积布局偏移cls优化)
  - [交互延迟（INP）优化](#交互延迟inp优化)
  - [性能监控](#性能监控)
- [安全考虑](#安全考虑)
  - [XSS 防护](#xss-防护)
  - [依赖安全](#依赖安全)
...
- [发布配置](#发布配置)
  ...
  - [pnpm link 测试流程](#pnpm-link-测试流程)
  - [Vite 配置要点](#vite-配置要点)
  - [ESLint 配置](#eslint-配置)
  - [Prettier 配置](#prettier-配置)
  - [lint-staged 配置](#lint-staged-配置)
  - [Husky Git Hooks](#husky-git-hooks)
- [正确性属性](#正确性属性)
  - [属性 1: 尺寸换算一致性](#属性-1-尺寸换算一致性)
  - [属性 2: 场景索引边界安全](#属性-2-场景索引边界安全)
  - [属性 3: 动画延迟传递性](#属性-3-动画延迟传递性)
  - [属性 4: 图片加载进度单调性](#属性-4-图片加载进度单调性)
  - [属性 5: 相对定位累加性](#属性-5-相对定位累加性)
  - [属性 6: Drag Release 连续性（transaction 同源 seed，双轨并行）](#属性-6-drag-release-连续性transaction-同源-seed双轨并行)
  - [属性 7: Drag ownership 与 re-grab 可逆性](#属性-7-drag-ownership-与-re-grab-可逆性)
  - [属性 8: 循环依赖不可达性](#属性-8-循环依赖不可达性)
  - [属性 9: 拖拽进度与动画进度一致性](#属性-9-拖拽进度与动画进度一致性)
  - [属性 10: Drag 非场景驱动动画只在正式到场后启动](#属性-10-drag-非场景驱动动画只在正式到场后启动)
  - [属性 11: 动画帧率稳定性](#属性-11-动画帧率稳定性)
  - [属性 12: Bundle 大小限制](#属性-12-bundle-大小限制)
  - [属性 13: 测试覆盖率要求](#属性-13-测试覆盖率要求)
  - [属性 14: Drag 映射与延迟门控正确性（`unit + scale`）](#属性-14-drag-映射与延迟门控正确性unit--scale)
  - [属性 15: Drag 模式智能阈值单调性](#属性-15-drag-模式智能阈值单调性)
  - [属性 16: Drag 回弹映射对称性](#属性-16-drag-回弹映射对称性)
  - [属性 17: Drag 模式场景状态机正确性](#属性-17-drag-模式场景状态机正确性)
  - [属性 18: Drag 模式边界反弹限制](#属性-18-drag-模式边界反弹限制)
  - [属性 19: Drag 双向目标尺子与回调终态一致性](#属性-19-drag-双向目标尺子与回调终态一致性)
  - [属性 20: useTransform 映射连续性](#属性-20-usetransform-映射连续性)
