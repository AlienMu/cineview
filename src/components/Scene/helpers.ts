...
export function normalizeSceneProps(props: SceneInternalProps): NormalizedSceneProps {
  ...
  return {
    ...
    globalViewportHeight: props.sceneRuntime?.viewportHeight ?? 0,
    globalDragProgress: props.dragRuntime?.progress ?? 0,
    globalRenderProgress: props.dragRuntime?.renderProgress ?? 0,
    globalDragTimelineProgress: props.dragRuntime?.timelineProgress ?? 0,
    globalIsDragging: props.dragRuntime?.isDragging ?? false,
    globalDragRelease: props.dragRuntime?.release ?? null,
    globalScrollProgress: props.scrollRuntime?.progress ?? 0,
    globalIsScrolling: props.scrollRuntime?.isScrolling ?? false,
    globalScrollDirection: props.scrollRuntime?.direction ?? null,
    globalScrollTransitionSnapshot: props.scrollRuntime?.transitionSnapshot ?? null,
    globalScrollBackdropActive: props.scrollRuntime?.backdropActive ?? false,
    globalScrollTimelineState: props.scrollRuntime?.timelineState ?? null,
    globalScrollActiveSceneIndex: props.scrollRuntime?.activeSceneIndex ?? 0,
    globalScrollViewportOffset: props.scrollRuntime?.viewportOffset ?? 0,
