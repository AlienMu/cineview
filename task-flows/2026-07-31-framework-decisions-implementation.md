...
- [x] D10 仅暂存 commit boundary 内文件，审计 staged diff 后创建可复现提交；记录 commit SHA，不推送。

## Status

`COMPLETE_WITH_ARCHIVED_P2_AND_QUESTIONS` — D0-D10 完成。可复现 framework snapshot 无已确认 P0/P1；Q2/Q4 与三个非阻断 registry P2 已归档。未推送。
...
- D8 static：独立复核确认 Q5/Q6 两项候选 P1 均关闭。保留三个非阻断 P2：`registerZoneAnimation` 不携带 `sceneIndex`，绕过 root preflight 的异常内部调用理论上可注入 budget；zone owner 以 `sceneIndex` 而非 generation lease 标识，非标准 root 生命周期下同 index stale cleanup 理论上仍可能命中；`warnedDuplicateZoneIdsRef` 在合法 unregister/reuse 后不清除，同 ID 后续再次冲突不会重复发 defensive dev warning。标准公共入口已有 root preflight、owner object 与 sceneIndex 防线，第三项只影响开发期重复告警，不影响生产 ownership 或错误回调；当前均无 P0/P1 红证。
...
- D9 snapshot：branch `codex/drag-release-dual-gate`，baseline HEAD `f164470d2ca4595ba7e6f0cbad455a11da763d82`。
- D10 reproducible fingerprint：framework-only commit `a4005a9ae9f9ef1ae2f133dd31af7ea16ba9e49a`，tree `f69a9cc2e3dec05c0e2f466a8a247255e916b6d1`。提交包含 160 个 framework boundary 文件，不含 `site/`；用户原先暂存的 5 个站点删除保持暂存。此任务流用独立 docs closeout commit 记录该 SHA，避免提交记录自身 SHA 的循环不可能问题。
