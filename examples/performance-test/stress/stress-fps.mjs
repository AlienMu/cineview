...
async function runDrag(browser) {
  ...
  const touchPath = async (fromY, toY, steps, stepMs, hold = false) => {
    await cdp.send('Input.dispatchTouchEvent', {
      type: 'touchStart',
      touchPoints: [{ x: X, y: fromY, id: 1, radiusX: 10, radiusY: 10, force: 1 }],
    });
...
async function runScroll(browser) {
  ...
  const readScroll = () =>
    page.evaluate(
      () => document.querySelector('[data-cineview-container="true"]')?.scrollTop ?? window.scrollY
    );
  const readMax = () =>
    page.evaluate(() => {
  ...
  const readProbe = () =>
    page.evaluate(() => ({
      ...
      videoA:
        document.querySelector('[data-cineview-animate-id="sa-video-0"] video')?.currentTime ??
        null,
      videoB:
        document.querySelector('[data-cineview-animate-id="sb-video-0"] video')?.currentTime ??
        null,
    }));
  ...
  const wheelTo = async (fraction) => {
    ...
    let guard = 0;
    while ((await readScroll()) < target - 60 && guard < 600) {
      await page.mouse.wheel(0, 110);
      await sleep(16);
      guard += 1;
    }
    while ((await readScroll()) > target + 60 && guard < 1200) {
      await page.mouse.wheel(0, -110);
      await sleep(16);
      guard += 1;
    }
...
(async () => {
  ...
  for (const mode of ['drag', 'scroll']) {
    ...
    summary[mode] = Object.fromEntries(
      Object.entries(phases).map(([p, gaps]) => [p, statsFor(gaps)])
    );
  }
