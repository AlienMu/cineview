...
In drag mode every scene is a viewport-filling card that the framework positions absolutely and slides by whole screens. That settles a good deal of the layout for you, and leaves some of the props you write with nothing to do.
...
That `height` row is where the same JSX breaks when moved between modes: a drag scene is one screen tall, while a scroll scene takes its height from content. Porting a drag page to scroll collapses scenes that relied on `100vh`.
...
## Styles the framework hardcodes
...
| Each scene frame | `z-index: 10` (current) / `1` (others)                    | Cross-scene layering is not yours to set                                  |
...
A scene's `layout.width` / `layout.height` land on the Scene element, but the Scene sits inside that `inset: 0` full-screen cell. So a sub-viewport scene is "placed somewhere within a full screen cell": you are sizing the visible card, not the paging step.
...
`transition.exitDuration` from the same group still applies, though: it feeds the resolved scene transition duration, and that duration is the numerator of the exit scrub (`renderProgress × sceneTransitionDuration`); the divisor is the element's own `duration.exit`. So raising `exitDuration` makes the exit scrub run faster, not slower. Treat this group as half-ignored, not wholly ignored.

**`stack.mode` has no effect in drag.** Only the scroll engine consumes it; in drag it takes part in no computation. `stack.zIndex` lands on the Scene element, but cross-scene layering is decided by the scene frame listed under "Styles the framework hardcodes."
...
`*` is likewise inert in drag. The one exception runs the other way: `firstSceneTimeout` also governs the scroll cold-start gate, see [Preloading](/docs/02-preload).
