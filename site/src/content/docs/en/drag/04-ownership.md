...
Step 3 is the one you run into in practice, so it gets its own section.

## The prepared snapshot: a precondition for dragging

Once a mounted scene has resolved all of its presets and its animation registry has settled, it publishes an immutable prepared snapshot to the root, containing the resolved mapping, a registry snapshot (each element's accumulated delay and the scene's total timeline length) and the visual variants.

Without that snapshot, a gesture aimed at the scene is silently refused: no signal at all in production, only a development warning. Publication waits for a macrotask plus every asynchronous preset parse, so:

**The first few gestures after mount can do nothing, and the framework offers no public "ready to drag" callback.** If your first screen needs to prompt the user to swipe, tie the prompt to `onLoadProgress` reaching 100 or a short delay of your own, rather than assuming mount implies draggable.
...
## Re-grab: take over in place with no visual jump
...
The takeover semantics are **stop the movement where it is and continue following the finger from that frozen position**:
...
The candidate-stage pause is reversible: if the press never acquires ownership (a tap, a cancel, a rejection), the original continuation resumes for its remaining duration and the commit timer is re-armed. Only genuine ownership preempts permanently.
