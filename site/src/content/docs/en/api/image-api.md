...
- **Visibility is not blocked by preloading.** The `<img>` renders on mount; the pipeline manages the network and cache hits, not the visibility of ordinary content — orchestrate "show when loaded" with `Animate`'s visibility / timeline semantics.
