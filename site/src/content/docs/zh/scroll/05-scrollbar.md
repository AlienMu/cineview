...
scroll 模式内置自绘滚动条覆盖层。开启即隐藏原生 gutter；rail 与 thumb 均为标准 DOM 元素，外观支持逐字段主题化配置。
...
下列每个字段都在覆盖层组件内解析，默认值与边界范围限定在此生效。
...
三处内置规则不可配置：thumb 长度下限为 40px（rail 本身更短时以 rail 为限）；当内容高度未超出可视区域（可滚动跨度不足 1px）时覆盖层整体不渲染；rail 的外描边、投影与键盘聚焦描边均为固定样式。
...
按在 thumb 上开始的手势进入拖拽，按在 rail 空白处是一次跳转（不进入拖拽）。拖拽产生的目标偏移与其它输入路径遵循相同的锁定区间边界约束，因此拖拽滚动条同样不会跳过锁定段。同时只有一个指针拥有 thumb，多点触控输入不会冲突打断。
