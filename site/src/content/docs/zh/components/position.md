...
- 元素要按设计稿坐标（设计 px，单尺子换算）放在 Scene 内的确定位置。
- 需要相对链排版：多个元素基于上一个 Position 的坐标累加偏移。
- scroll 模式下需要「跟随滚动固定在视口、但只在本场景内」的覆盖层（`layer.fixed`，scene-scoped，绝不跨章节漂浮）。

## 坐标与 fixed layer

坐标写在 `at` 上（`x`/`y`/`offsetX`/`offsetY`/`anchor`），都是设计像素；fixed 覆盖层用 `layer.fixed` 声明，作用域限定在其 Scene 内，绝不跨章节漂浮。
