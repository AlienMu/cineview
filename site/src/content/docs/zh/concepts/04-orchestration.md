...
把多个元素排出先后靠两件东西：`after` 让元素之间排队，`stagger` 让一个容器内的子元素逐个揭示。两者都在绘制第一帧前编译好，不靠运行时互等。
...
同样一行 `after`，跟随滚动的元素和可见性驱动的元素走的是两套互不相干的实现。动手排时间线之前，先确认元素由什么驱动。
...
在「after 链」那个例子里，`cta` 的启动时间 = title 入场时长 + sub.delay(100) + sub 入场时长 + cta.delay，一次算完。
...
为什么不能复用累计延迟：可见性驱动的每个元素各自进入视口才起跑，没有共享原点。把 leader 的 delay 加时长再算一遍，等于凭空多等一整段。
...
| 可见性驱动    | 可见性驱动       | 允许                                   |
| 跟随滚动      | 同为跟随滚动     | 允许                                   |
| 可见性驱动    | scroll 锁定区    | 允许（运行时按 leader 完成过入场判断） |
| scroll 锁定区 | 可见性驱动       | 拒绝                                   |
| drag          | 任何其他驱动方式 | 拒绝                                   |

拒绝的方向报 `INVALID_ANIMATION`（内部原因 `incompatible-driver`）并**跳过这条依赖**，follower 仍按自身的可见性条件与 delay 运行。

理由是两条时钟对不上：zone 的预算是确定的 `1ms = 1px`，而可见性驱动的完成时刻取决于用户何时把元素滚进视口，没有任何滚动坐标可以对应它。反过来可以，因为「scroll leader 越过入场终点」这件事本身是一个可以观察到的运行时事实。
...
设了 `stagger` 后，`children` 必须是单个 ReactElement，其中每个直接子元素由 framer 原生 `staggerChildren` 逐个揭示，子元素用 `enterAnimation` 的变体。
...
| 离场方向               | 实际播什么                                                                  |
| ---------------------- | --------------------------------------------------------------------------- |
| 正向（往下一屏拖走）   | `exitAnimation` 的变体，从当前态插值到退场态                                |
| 反向（往上一屏拖回）   | 入场动画倒放（`initial → animate` 取 `1 - 进度`），完全不看 `exitAnimation` |
| 未声明 `exitAnimation` | 正向离场也什么都不播，元素保持终态不动，只有页面在滑                        |
